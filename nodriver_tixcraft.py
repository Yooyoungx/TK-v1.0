...
CONST_APP_VERSION = "MaxBot (2024.08.05)"
...
async def nodriver_kktix_signin(tab, url, config_dict):
    ...
    if kktix_password == "":
        kktix_password = util.decrypt_me(
...
async def nodriver_goto_homepage(driver, config_dict):
        ...
        if len(tixcraft_sid) > 1:
            ...
            if not is_cookie_exist:
                new_cookie = cdp.network.CookieParam(
                    "SID", tixcraft_sid, domain=domain_name, path="/", http_only=True, secure=True)
    ...
    if 'ibon.com' in homepage:
        ...
        if len(ibonqware) > 1:
            ...
            if not is_cookie_exist:
                new_cookie = cdp.network.CookieParam(
                    "ibonqware", ibonqware, domain=".ibon.com.tw", path="/", http_only=True, secure=True)
                cookies.append(new_cookie)
            await driver.cookies.set_all(cookies)
    ...
    if not registrationsNewApp_div is None:
        ...
        if is_dom_ready:
            ...
            if is_ticket_number_assigned:
                ...
                last_sent_minute = util.optimized_email_sending(
                    config_dict, "ticket", last_sent_minute, url)
...
async def nodriver_tixcraft_main(tab, url, config_dict, ocr, Captcha_Browser):
    ...
    if not 'tixcraft_dict' in globals():
        tixcraft_dict = {}
        tixcraft_dict["last_sent_minute"] = 0
    ...
    if '/ticket/ticket/' in url:
        ...
        last_sent_minute = tixcraft_dict.get("last_sent_minute", 0)
...
async def nodriver_ticketplus_account_sign_in(tab, config_dict):
    ...
    if ticketplus_password == "":
        ticketplus_password = util.decrypt_me(
                                    ...
                                    for col_12 in lv2.children:
                                        if col_12.local_name == 'div':
                                            ...
                                            for d_flex in col_12.children:
                                                ...
                                                if len(row_text) > 0:
                                                    row_text = util.format_keyword_string(
                                                        row_text)
                                                    ...
                                                    if len(date_keyword_item) > 0:
                                                        ...
                                                        for date_keyword in date_keyword_array:
                                                            date_keyword = util.format_keyword_string(
                                                                date_keyword)
                            ...
                            if ticket_select:
                                ...
                                if selected_ticket_string:
                                    if len(selected_ticket_string) > 0:
                                        selected_ticket_value = int(
                                            selected_ticket_string)
        ...
        if is_event_page:
            if is_event_page:
                ...
                if is_finish_checkbox_click:
                    ...
                    ibon_dict["last_sent_minute"] = util.optimized_email_sending(
                        config_dict, "ticket", ibon_dict["last_sent_minute"], url)
